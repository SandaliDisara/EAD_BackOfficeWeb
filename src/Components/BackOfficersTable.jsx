...
const BackOfficersTable = () => {
  ...
  const fetchBackOfficers = async () => {
    try {
      const response = await axios.get('http://192.168.51.122:5228/api/BackOfficer'); // Update URL based on your API
  ...
  const handleDelete = async (id, role) => {
    try {
      ...
      if (role === 'Vendor') {
        await axios.delete(`http://192.168.51.122:5228/api/Vendor/${id}`); // Update with the correct Vendor API endpoint
      }

      // Delete the back officer from the BackOfficer collection
      await axios.delete(`http://192.168.51.122:5228/api/BackOfficer/${id}`); // Update URL based on your API
