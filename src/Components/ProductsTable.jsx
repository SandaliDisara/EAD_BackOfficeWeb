...
const ProductsTable = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('http://192.168.51.122:5228/api/products'); // Update the URL based on your API
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://192.168.51.122:5228/api/products/${id}`); // Update the URL based on your API
